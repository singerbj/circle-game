...
var createAndAddThing = function(){
    ...
    var thing = Circle(rand(0, 800), rand(0, 600), rand(10, 40), {
        color: 'blue',
...
var createAndFireBullet = function(x, y){
    ...
    var bullet = Circle(player.x, player.y, 3, {
        playerId: player.id,
        color: 'orange',
        beforeUpdate: function(){
            if(!this.timeAlive) this.timeAlive = 0;
...
var update = function(dt) {
    ...
    for(e in entities){
        entityCount += 1;
        if(entities[e].beforeUpdate) entities[e].beforeUpdate();
    }
    ...
    for(e in entities){
        if(entities[e].afterUpdate) entities[e].afterUpdate();
    }
};
